...
usage()
{
	echo "usage: $0 -c <name of your folder> [ -i <input dir> -o <output_dir> -f <output_name> ]"
	exit 0
}
...
if [ X${RUNMODE} = Xreal ]; then
	...
	echo "DRY-RUN: java -jar ./chilo-epub3-maker.jar -publish html -course ${COURSE} -input-path ${INPUT_DIR} -output-path ${OUTPUT_DIR} ${ONAME_OPT}"
fi
